...
function input(event:KeyboardEvent):Void {
  ...
  if (!game.controls.controllerMode && (key > -1 || key < game.playerStrums.length)) {
    ...
    if (game.controls.justPressed(inputFromArray)) {
      inputPress(key);
    } else if (game.controls.justReleased(inputFromArray)) {
      inputRelease(key);
    }
  }
  return;
    ...
    if (sortedNotesList.length > 1) {
      for (epicNote in sortedNotesList) {
        if (epicNote != coolNote && epicNote.strumTime - coolNote.strumTime <= 12.5 * 100 / Conductor.bpm) {
          if (game.guitarHeroSustains && epicNote.tail.length != 0) {
            for (tail in epicNote.tail) {
              if (tail != null) {
